...
- `SeleconPortalDevStack` (`lib/selecon-portal-stack.ts`): RDS PostgreSQL, ElastiCache
  (Valkey), bucket S3 de uploads, Secrets Manager, roles IAM, log groups, e os serviços
  ECS de **api** e **worker** (que ainda não existem — o serviço **web** já existe e
  continua fora do controle deste stack, apenas referenciado).
- `SeleconPortalPipelineStack` (`lib/pipeline-stack.ts`): CodeBuild + CodePipeline,
  condicionada a uma CodeConnection já autorizada (ver bloqueio abaixo).
...
(gera cdk.out/SeleconPortalDevStack.template.json — exit code 0)

$ npx cdk synth SeleconPortalPipelineStack -c codeConnectionArn=<arn>
(gera cdk.out/SeleconPortalPipelineStack.template.json — exit code 0)
```
...
# Só quando houver credenciais AWS reais e a CodeConnection autorizada:
npx cdk deploy SeleconPortalDevStack -c albListenerArn=<arn-do-listener-http-do-alb>
